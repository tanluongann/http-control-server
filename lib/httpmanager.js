...
manager.init = function(app, dbmanager, callback) {
  ...
  app.post('/ips', function(req, res) {
    ...
    dbmanager.Device.getOrCreate(id, function(err, dev) {
      if (err || !dev) {
         console.log(id + " - Updating IP, error while fetching the device in DB");
      }
      else {
        if (dev.password === pw) {
          console.log(id + " - Updating IP, password OK");
        }
        else {
          console.log(id + " - Updating IP, password wrong (OK for debug)");
        }
